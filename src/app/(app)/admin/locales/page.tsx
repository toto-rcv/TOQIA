...
export const metadata = { title: "Locales · Toqia Admin" };
...
export default async function AdminLocationsPage({
  ...
  return (
    <>
      <PageHeader
        title="Locales"
        subtitle="Cada local tiene su propia página pública, sus pulseras y sus camareros."
      >
      ...
      {cuentas.length === 0 ? (
        ...
        <Card>
          <EmptyState>
            No hay locales{filtro ? " para esta cuenta" : ""}.
          </EmptyState>
        </Card>
      ) : (
          ...
          <Table>
            <Thead>
              <tr>
                ...
                <Th className="w-[110px] text-center">Reseñas</Th>
                <Th className="w-[100px] text-right">Pulseras</Th>
                ...
                <Tr key={local.id} className={local.active ? undefined : "opacity-60"}>
                  ...
                  <Td className="text-center">
                    {local.googleReviewUrl ? (
                      <Badge tone="active">cargado</Badge>
                    ) : (
                      <Badge tone="warning" title="La landing no muestra el botón de reseña">
                        falta
                      </Badge>
                    )}
                  </Td>
                  ...
                  <Td className="num text-right text-sm text-ex-text">
                    {formatNumber(local.braceletCount)}
                  </Td>
                  <Td className="num text-right text-sm text-ex-text">
                    {formatNumber(local.scanCount)}
                  </Td>
                  ...
                  <Td>
                    <div className="flex items-center justify-end gap-1.5">
                      <Link
                        ...
                        aria-label="Ver la página pública"
